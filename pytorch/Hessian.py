...
class hessian:
    ...
    def __init__(self, model, criterion, data=None, dataloader=None, cuda=True):
        ...
        self.inputs, self.targets = self.data[0]
        ...
        for i in range(len(gradsH)):
            if type(gradsH[i]) is not float:
                grad.append(gradsH[i])
                par.append(params[i])
        self.params = par
        self.gradsH = grad  # gradient used for Hessian computation

...
def uproot_tree_to_numpy_LEGACY(fname, inbranches_listlist, nMaxlist, nevents, treename="deepntuplizer/tree", stop=None, branches=None, flat=True):
   ...
   #                 stop = None,
    #                branches = branches
     #           )
                ...
                counter=counter+1
            weighter.createRemoveProbabilitiesAndWeights(self.referenceclass)
            weighter.printHistos('/afs/cern.ch/user/a/anstein/dj/Flatten') #If you need to print the 2D histo, choose your output dir
            return {'weigther':weighter}
...
#        out = pd.DataFrame(arr, columns=['isB', 'isBB', 'isLeptB', 'isC', 'isUDS', 'isG', 'jet_pt', 'jet_eta']+lab_names)
        ...
        out_numpy = np.core.records.fromarrays(np.vstack( (predicted[0].transpose(),truth[0].transpose(), features[0][:,0:2].transpose() ) ),
                                         names='prob_isB, prob_isBB,prob_isLeptB, prob_isC,prob_isUDS,prob_isG,isB, isBB, isLeptB, isC,isUDS,isG,jet_pt, jet_eta')
...
class TrainData_DF_Run2_backup(TrainData):
    def __init__(self):
        ...
        self.npf_branches = ['Npfcan_ptrel', 

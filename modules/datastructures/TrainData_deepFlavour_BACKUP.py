...
def uproot_tree_to_numpy_LEGACY(
    fname,
    inbranches_listlist,
    nMaxlist,
    nevents,
...
class TrainData_DeepCluster(TrainData):
    def __init__(self):
        ...
        self.remove = True
        self.referenceclass = (
            "isB"  # Choose 'flatten' for flat or one of the truth branch for ref
        )
        self.red_classes = [
            "cat_B",
            "cat_C",
        ...
        self.class_weights = np.array(
            [1.00, 1.00, 2.50, 5.00], dtype=float
    ...
    def createWeighterObjects(self, allsourcefiles):
        ...
        if self.remove:
            for fname in allsourcefiles:
                ...
                nparray = root2array(
                    fname, treename="deepntuplizer/tree", stop=None, branches=branches
                )
                norm_hist = True
            ...
            weighter.printHistos(
                "/afs/cern.ch/user/a/ademoor/Flatten/"
            )  # If you need to print the 2D histo, choose your output dir
    ...
    def convertFromSourceFile(self, filename, weighterobjects, istraining):
        ...
        if self.remove:
            ...
            for_remove = root2array(
                filename, treename="deepntuplizer/tree", stop=None, branches=b
            )
    ...
    def writeOutPrediction(
        ...
        out = np.core.records.fromarrays(
            np.vstack(
                (
                    predicted[0].transpose(),
                    truth[0].transpose(),
                    features[0][:, 0:2].transpose(),
                )
            ),
...
class TrainData_DeepVertex(TrainData):
    def __init__(self):
        ...
        self.weight_binX = np.array(
            [100, 125, 150, 175, 200, 250, 300, 400, 500, 600, 2000], dtype=float
        )
        # self.weight_binX = (np.exp(np.linspace(np.log(15), np.log(1000), 16))).tolist()
        # self.weight_binX = np.array([10,25,30,35,40,45,50,60,75,100,125,150,175,200,250,300,400,500,600,2000],dtype=float) #Ref reweighting
        self.weight_binY = np.array(
...
class TrainData_ParticleNet(TrainData):
    def __init__(self):
        ...
        self.cpf_pts_branches = ["Cpfcan_etarel", "Cpfcan_phirel"]

        self.npf_pts_branches = ["Npfcan_etarel", "Npfcan_phirel"]

        self.vtx_pts_branches = ["sv_etarel", "sv_phirel"]
...
class TrainData_DFE(TrainData):
    def __init__(self):
        ...
        self.weight_binY = np.array(
            [-2.5, -2.0, -1.5, -1.0, -0.5, 0.5, 1, 1.5, 2.0, 2.5], dtype=float
        )
    ...
    def convertFromSourceFile(self, filename, weighterobjects, istraining):
        ...
        x_cpf = MeanNormZeroPadParticles(
            filename, None, self.cpf_branches, self.n_cpf, self.nsamples
        )

        x_npf = MeanNormZeroPadParticles(
            filename, None, self.npf_branches, self.n_npf, self.nsamples
        )

        x_vtx = MeanNormZeroPadParticles(
            filename, None, self.vtx_branches, self.n_vtx, self.nsamples
        )

        cpf_pts = MeanNormZeroPadParticles(
            filename, None, self.cpf_pts_branches, self.n_pts, self.nsamples
        )

        npf_pts = MeanNormZeroPadParticles(
            filename, None, self.npf_pts_branches, self.n_pts, self.nsamples
        )
...
class TrainData_ParT(TrainData):
    ...
    def createWeighterObjects(self, allsourcefiles):
        ...
        if self.remove:
            ...
            weighter.createRemoveProbabilitiesAndWeights(self.referenceclass)
            weighter.printHistos(
                "/afs/cern.ch/user/a/anstein/dj/Flatten"
            )  # If you need to print the 2D histo, choose your output dir
            return {"weigther": weighter}
    ...
    def convertFromSourceFile(self, filename, weighterobjects, istraining):
        ...
        x_global = uproot_tree_to_numpy(
            urfile, self.global_branches, 1, self.nsamples, flat=True
        )

        x_cpf = uproot_tree_to_numpy(
            urfile, self.cpf_branches, self.n_cpf, self.nsamples, flat=False
        )

        x_npf = uproot_tree_to_numpy(
            urfile, self.npf_branches, self.n_npf, self.nsamples, flat=False
        )

        x_vtx = uproot_tree_to_numpy(
            urfile, self.vtx_branches, self.n_vtx, self.nsamples, flat=False
        )

        cpf_pts = uproot_tree_to_numpy(
            urfile, self.cpf_pts_branches, self.n_cpf, self.nsamples, flat=True
        )

        npf_pts = uproot_tree_to_numpy(
            urfile, self.npf_pts_branches, self.n_npf, self.nsamples, flat=False
        )

        vtx_pts = uproot_tree_to_numpy(
            urfile, self.vtx_pts_branches, self.n_vtx, self.nsamples, flat=False
        )
    ...
    def writeOutPrediction(
        ...
        out_numpy = np.core.records.fromarrays(
            np.vstack(
                (
                    predicted[0].transpose(),
                    truth[0].transpose(),
        ...
        np.save(outfilename.strip(".root") + ".npy", out_numpy)


# only 4 SVs, and no etarel or phirel for npf / sv
class TrainData_DF_Run2_backup(TrainData):
    def __init__(self):
        TrainData.__init__(self)
...
class TrainData_DeepCSV(TrainData):
    ...
    def createWeighterObjects(self, allsourcefiles):
        ...
        nparray = self.readTreeFromRootToTuple(
            allsourcefiles,
            branches=self.vtx_branches
            + self.eta_rel_branches
            + self.track_branches
            + self.global_branches,
            limit=500000,
        )
    ...
    def writeOutPrediction(
        ...
        out = np.core.records.fromarrays(
            np.vstack((predicted[0].transpose(), truth[0].transpose(), spectators)),
            names="prob_isB, prob_isBB, prob_isC,prob_isUDSG,isB, isBB, isC,isUDSG,jet_pt, jet_eta",
        )
        array2root(out, outfilename, "tree")


# A datastructure that is almost identical to DeepJet, with the difference of having only the four output nodes as DeepCSV
class TrainData_DF_DeepCSV_outputs(TrainData):
    def __init__(self):
        TrainData.__init__(self)
